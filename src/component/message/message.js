...
import { Badge ,Icon,Button,Modal,Input , Radio,message,Card,Avatar  ,Slider} from 'antd';
...
@connect(
    state => state,
    {authDataSuccess,getMessageUnReadNum,listeningLatestMessageList,getLatestMessageList}
)
class Message extends React.Component{
    ...
    goToChat(send){
        this.props.history.push(`/chat/${send}`)// 和发送方聊天
        // this.props.chat.chatTarget = send
    }
    ...
    deleteMessage(){
        ...
        // console.log($(this).parent('div.sl-opts'))
        // console.log($(this).parents($('div.sl-li')[0]).css({display:'none'}))
        // console.log($('li').data('datamessagelistid'))
    }
    ...
    render(){
        ...
        return(
            <div>
                ...
                <div className="message-list">
                    ...
                    <div className="chat-list">
                        {this.props.message.messageList.map( v => {
                            ...
                            const send = v.message.send === _id ? v.message.receive : v.message.send

                            return (
                                <div className="message" onClick={()=>this.goToChat(send)} key={v.message._id}>
                                    <div className="avatar" style={{float:'left'}}>
                                        <Badge showZero={false} count={v.message.unReadNum}>
                                            <img src={v.avatarName} style={{width:'50px',height:'50px',borderRadius:'8px'}}  alt=""/>
                                        </Badge>
                                    </div>
                                    <span className="nickName">{v.message.nickName}</span>
                                    <span className="chatText">{v.message.chatText}</span>

                                    <span className="sendTime">{this.showChatDate(v.message.sendTime)}</span>
                                </div>
                            )
                        })}
                    </div>
                ...
                {/*<ul>*/}
                        {/*<li className="sl-li ui-border-b" datamessagelistid="123">*/}
                            ...
                            {/*<div className="sl-opts">*/}
                                {/*<span onClick={() => this.deleteMessage()} style={{backgroundColor: '#F95F61', color:' #fff'}}>删除</span>*/}
                                {/*/!*<span style={{backgroundColor: '#ff9c00',color: '#fff'}}>分享</span>*!/*/}
                            {/*</div>*/}
                        {/*</li>*/}
                ...
                <div className="footerLink">
                    ...
                    <a id='a-message' onClick={ () => this.goToMessagePage()}  className="active">
